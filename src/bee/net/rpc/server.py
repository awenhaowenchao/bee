...
from bee.net.rpc.registry.registry import Server as RegistryServer

from bee.net.rpc.channel import Channel
from bee.net.rpc.codec import RequestHead, IServerCodec, Response, ResponseHead, Result, \
    Stream, get_codec
...
class Server():
    ...
    def init_codec_builder(self):
        if self.opts.macher == "http":
            ...
            b = get_codec("json")
            if not b:
                import bee.net.rpc.codecs.json.json as json
                json.init()
        else:  # proto
            b = get_codec("proto")
            if not b:
                import bee.net.rpc.codecs.proto.proto as proto
                proto.init()
